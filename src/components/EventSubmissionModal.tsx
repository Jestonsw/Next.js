            ...
            <div>
              ...
              <div className="relative">
                <input
                  type="tel"
                  required
                  value={formatPhoneDisplay(submitterInfo.submitterPhone.replace(/[^0-9]/g, ''))}
                  onChange={() => {}} // Dummy onChange for React
                  onKeyDown={(e) => {
                    if (e.key >= '0' && e.key <= '9') {
                      e.preventDefault()
                      const currentDigits = submitterInfo.submitterPhone.replace(/[^0-9]/g, '')
                      if (currentDigits.length < 10) {
                        setSubmitterInfo({...submitterInfo, submitterPhone: currentDigits + e.key})
                      }
                    } else if (e.key === 'Backspace') {
                      e.preventDefault()
                      const currentDigits = submitterInfo.submitterPhone.replace(/[^0-9]/g, '')
                      if (currentDigits.length > 0) {
                        setSubmitterInfo({...submitterInfo, submitterPhone: currentDigits.slice(0, -1)})
                      }
                    }
                  }}
                  onFocus={(e) => {
                    setTimeout(() => {
                      const phoneLength = submitterInfo.submitterPhone.replace(/[^0-9]/g, '').length
                      let cursorPos = 1
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-edirne-500 font-mono"
