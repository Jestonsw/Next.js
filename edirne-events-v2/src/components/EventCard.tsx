...
const EventCard = memo(function EventCard({ event, category, isFavorite, onFavoriteToggle, onEventClick }: EventCardProps) {
  ...
  const getImageSrc = useCallback((imageUrl: string) => {
    ...
    // FORCE ALL UPLOADS to load from replit.app for cross-domain compatibility
    if (imageUrl.startsWith('/uploads/')) {
      // Use stable URL without random parameters for caching
      const replitUrl = `https://edirne-events.replit.app${imageUrl}`
      imageCache.set(cacheKey, replitUrl)
      console.log(`🎯 CACHED: Event ${event.id} - ${replitUrl}`)
      return replitUrl
    }
